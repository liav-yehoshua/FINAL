...
## ⚠️ **חשוב! קובץ ההרשאות חסר**

**הקובץ `google-credentials.json` חסר ב-Git מסיבות אבטחה.**

### איך להשיג את הקובץ:
1. **הורד את קובץ ה-ZIP** שניתן לך בנפרד
2. **חלץ את הקובץ** באמצעות הסיסמה: `final`
3. **העתק את `google-credentials.json`** לתיקיית הפרויקט
4. **ודא שהקובץ נמצא** באותה תיקייה כמו `exam_grader.py`

**ללא קובץ זה המערכת לא תעבוד!**
...
- [התקנה והגדרות](#התקנה-והגדרות)
  - [הכנת קובץ הגדרות סביבה (.env)](#1-הכנת-קובץ-הגדרות-סביבה-env)
  - [הכנת קובץ הרשאות Google Cloud](#2-הכנת-קובץ-הרשאות-google-cloud)
  - [התקנת חבילות](#3-התקנת-חבילות)
  - [חבילות נדרשות](#4-חבילות-נדרשות)
...
### 1. **הכנת קובץ הגדרות סביבה (.env)**
1. העתק את קובץ הדוגמה:
   ```bash
   cp .env.example .env
   ```
2. ערוך את הקובץ `.env` והזן את הנתונים האמיתיים:
   ```env
...
### 2. **הכנת קובץ הרשאות Google Cloud:**
   - הורד את קובץ ההרשאות (`google-credentials.json`) מחשבון Google Cloud שלך
   - הנח אותו בתיקיית הפרויקט
   - ודא שהקובץ מכיל הרשאות ל-Vision API
...
2. **קובץ google-credentials.json:**
   ...
   - הפעילו את Google Vision API
   - צרו Service Account, תנו לו הרשאות ל-Vision API, והורידו את קובץ ה-JSON
   - שימו את הקובץ בשם `google-credentials.json` בתיקיית הפרויקט
...
- העתק `.env.example` ל-`.env` וערוך עם הנתונים האמיתיים שלך. הקובץ לא יועלה ל-Git אוטומטית.
